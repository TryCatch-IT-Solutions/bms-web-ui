...
export const LogoForm = () => {
    ...
    const apiForm = useForm<UploadLogoType>({
        mode: 'all',
        resolver: logZodResolver(uploadLogoSchema),
    })
    ...
    const { data: apiKey, isLoading } = useQuery({
        ...
        queryFn: () => getAPIKey(API_KEY_LABELS.DYNAMIC_LOGO, 0),
    })
    ...
    useEffect(() => {
        if (apiForm.getValues('key') !== API_KEY_LABELS.DYNAMIC_LOGO) {
            setValue('key', API_KEY_LABELS.DYNAMIC_LOGO)
        }
    }, [isLoading])
            ...
            <Form {...apiForm}>
                ...
                >
                    <CardHeader className='flex flex-row justify-between'>
                        <p className='font-semibold text-bms-gray-500'>Logo</p>
                        <Button
                            type='button'
